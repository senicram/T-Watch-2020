    ...
    while (lunokIoT_BLEMonitorTaskLoop) {
        ...
        if (millis() > BLEMonitorTasknextBLEScan) {
            if (LoT().GetBLE()->IsEnabled()) {
                ...
                BLEScanResults foundDevices = pBLEScan->getResults(4, false);
...
bool BLEMonitorApplication::Tick() {
    ...
    uint16_t iconColor = LoT().GetBLE()->IsEnabled() ? TFT_WHITE : TFT_DARKGREY;
    canvas->drawXBitmap((TFT_WIDTH - img_bluetooth_32_width) / 2,
                        (TFT_HEIGHT - img_bluetooth_32_height) / 2,
                        img_bluetooth_32_bits,
                        img_bluetooth_32_width,
                        img_bluetooth_32_height,
                        iconColor);

    if (BLEKnowDevices.size() > 0) {
        int elementDegrees = 360 / BLEKnowDevices.size();
        if (elementDegrees > 359) { elementDegrees = 0; } // avoid overflow
